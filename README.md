...
- **Galerie** — artistes, saints et œuvres depuis Directus (`https://api.incarnatio.ch`). Notices, personnes représentées, images (clés Directus `thumbnail` / `medium` / `large` / `xlarge`), filtres d’étiquettes (ET / OU) via paramètres GET.
...
| CMS / images | Directus public `https://api.incarnatio.ch` — collections `artists`, `artworks` |
...
- Header sticky : logo lys + Incarnatio, nav (Scriptorium en menu), **Qualité max**, sélecteur de thème, menu mobile
...
Par défaut : `https://api.incarnatio.ch` (surcharge via `DIRECTUS_URL`).
